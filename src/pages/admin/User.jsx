    ...
    <div className="container">
      ...
      <ul className="nav nav-tabs">
        <li className="nav-item">
          <a className={`nav-link ${showReportsTo ==="to" ? "active fw-bold" : "text-secondary"}`} data-show="to" href="" onClick={handleShowReports}>
            Về tài khoản ({total})
          </a>
        </li>
        <li className="nav-item" >
          <a className={`nav-link ${showReportsTo === "from" ? "active fw-bold" : "text-secondary"}`} href="" data-show="from" onClick={handleShowReports}>
  ...
  return (
    <div className="d-flex justify-content-between my-3 p-3 border border-1 rounded-3">
      <div>
        ...
        <p>Thời gian tạo: {new Date(Date.parse(user.created_at)).toLocaleString()}</p>
      </div>
